...
def LinearActivation(
    ...
    **kwargs,
    ...
    """Returns a linear nn.Module with control over axes order, initialization,\
          and activation."""
...
def power(L, A, v=None):
    ...
    Identity = torch.eye(A.shape[-1]).to(A)  # , dtype=A.dtype, device=A.device)

    powers = [A]
    length = 1
    while True:
        if L % 2 == 1:
            Identity = powers[-1] @ Identity
        L //= 2
        if L == 0:
            break
        length *= 2
...
def transition(measure, N, **measure_args):
    ...
    measure: the type of measure
      ...
      lagt, tlagt - previous versions of (tilted) \
        Laguerre with slightly different normalization
    """
    # Legendre (translated)
    if measure == "legt":
...
def nplr(measure, N, rank=1, dtype=torch.float, diagonalize_precision=True, B_clip=2.0):
    ...
    ) > 1e-5:  # if not torch.allclose(_A - _A[0,0]*torch.eye(N), \
        # torch.zeros(N, N), atol=1e-5):
...
def dplr(
    ...
    - B_init:(str)['constant' | 'random' | 'alternating' |\
          'unit-cw' | 'unit-ccw' | 'hippo']
    ...
    if init in ["random", "rand"]:
        ...
        if real_random:
            ...
            # The A matrix is diag(-1, -2, ..., -N),
            # which are the eigenvalues of the HiPPO matrix
    ...
    if B_random:
        log.warning(
            "'B_random' is deprecated in favor of B_init='random' and \
    ...
    # Experimental feature that appeared
    # in earlier versions of HTTYH (not extensively tested)
...
def combination(inits, N, R, S, **ssm_args):
    ...
    assert (
        S % len(inits) == 0
    ), f"{S} independent trainable SSM copies must be multiple of {len(inits)}\
          different inits"
...
def param_transform(param, transform="none"):
    ...
    if transform == "none":
        ...
        # JAX version seems to NaN if you allow 0's,
        #  although this code was fine without it
        p = F.relu(param) + 1e-4
    elif transform == "sigmoid":
        p = F.sigmoid(param)
    elif transform == "softplus":
...
class Kernel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        d_model (H): Model dimension,
          or number of independent convolution kernels created.
        channels (C): Extra dimension in the returned output (see .forward()).
            - One interpretation is that it expands
              the input dimension giving it C separate "heads" per feature.
              That is convolving by this kernel maps shape (B L D) -> (B L C D)
            - This is also used to implement
              a particular form of bidirectionality in an efficient way.
        ...
        l_max (L): Maximum kernel length (optional).
          If unspecified, most Kernel instantiations
        ...
        lr: Optional dictionary specifying special hyperparameters for .register().
            Passing in a number (e.g. 0.001)
              sets attributes of SSM parameters (A, B, dt).
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        Returns:
          ...
          - (B, H, L) (batch, d_model, l_kernel)
              Extra information for how the state \
                affects the output of convolving by kernel.
        """
        raise NotImplementedError
    ...
    def forward_state(self, u, state):
        """Forward the state through a sequence, i.e.
        computes the state after passing chunk through the kernel."""
        raise NotImplementedError

    @property
    def d_state(self):
        """Implement this for interfaces that want\
              to interact with a stateful layer (i.e. SSMs).

        Currently the only codepath that might use this is the StateDecoder, \
            which is not used.
        """
        raise NotImplementedError
...
class SSMKernel(Kernel):
    ...
    This class is abstract and only defines some initializations\
          and flags that are common to all SSM variants.
    It is instantiated by subclasses SSMKernel{Dense,Real,Diag,DPLR}.

    Options:
    d_state (N): State size (dimensionality of parameters A, B, C). \
    Generally shouldn't need to be adjusted and doens't affect\
          speed much for most kernels (e.g. S4, S4D).
    ...
    dt_tie: Keep dt tied across the N dimensions of the state. Although this \
        theoretically makes more sense, models such as S5 and \
            Mega have found slightly improvements by setting it to False.
    dt_transform: Transform function for parameterization of dt \
        (default 'softplus', used to be 'exp')

    rank: Rank of low-rank correction for DPLR mode. \
        Needs to be increased for init "legt".
    n_ssm: Number of independent trainable (A, B) SSMs, e.g.
        `n_ssm=1` means all A/B parameters are tied \
            across the H different instantiations of C.
        `n_ssm=None` means all H SSMs are completely independent.
        Generally, changing this option can save \
            parameters but doesn't affect performance or speed much.
        This parameter must divide H.
    init: Options for initialization of (A, B). \
    For DPLR mode, recommendations are "legs", "fout", "hippo" (combination of both).\
    For Diag mode, recommendations are "diag-inv", "diag-lin", "diag-legs", and "diag" \
                (combination of diag-inv and diag-lin).
    init_args: Extra arguments passed into \
        initialization function (see dplr.py for options).
    """
    ...
    def __init__(
        ...
        **init_args,
        ...
        if measure is not None:
            log.warning(
                "Warning: 'measure' option changed to 'init'\
...
class SSMKernelDiag(SSMKernel):
    ...
    backend: ['cuda' | 'keops' | 'naive'] Options for\
          Vandermonde/Cauchy kernel (in order of efficiency).
    is_real : Real-valued SSM; can be interpreted as EMA.
    """

    def __init__(
        self,
        disc: str = "zoh",  # Change to 'bilinear' to match S4,
        # but should make little difference either way
        dt_fast: bool = False,
...
class SSMKernelDPLR(SSMKernelDiag):
    """SSM kernel for diagonal + low rank (DPLR) state matrices,\
          corresponding to the original S4 model."""

    @torch.no_grad()
    def _setup_C(self, L):
        """Construct C~ from C.

        Two modes are supported: go directly to length L if self.l_kernel is 1, \
        ...
        if self.l_kernel.item() == 0:
            ...
            if self.verbose:
                log.info(
                    f"S4: Doubling length from L = {self.l_kernel.item()} \
                        to {2*self.l_kernel.item()}"
                )
            double_length = True
    ...
    def _omega(self, L, dtype, device, cache=True):
        ...
        This also caches a version of the nodes \
            "unprocessed" with the bilinear transform.
        This method should be called everytime \
            the internal length self.l_kernel changes.
        """
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        # Print out kernel lengths;
        # it can be tricky to make sure the length logic is correct
        if self.verbose:
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        # (done in forward pass so it's on the correct device)
        ...
        #  while we are asked to provide a kernel of
        # length L at (relative) frequency rate
        if L is None:
        ...
        if state is not None:
            ...
            # Can do this without expanding (maybe minor speedup \
            # using conj symmetry in theory),\ but it's easier to read this way
    ...
    def _step_state_linear(self, u=None, state=None):
        ...
        Unfortunately, as currently implemented it's about 2x slower because \
            it calls several sequential operations.
        Perhaps a fused CUDA kernel implementation would be much faster.

        u: (H) Input
        state: (H, N/2) State with conjugate pairs. Optionally,\
              the state can have last dimension N.
    ...
    def _step_state(self, u, state):
        """Must be called after self.default_state() \
            is used to construct an initial state!"""
    ...
    def _setup_step(self, mode="dense"):
        ...
        if self.l_kernel.item() == 0:
            ...
            Identity = torch.eye(self.dA.size(-1)).to(dA_L)

            dC = torch.linalg.solve(
                Identity - dA_L.transpose(-1, -2),
                C.unsqueeze(-1),
            ).squeeze(-1)
        self.dC = dC
    ...
    def default_state(self, *batch_shape):
        ...
        )  # Used in default_state, which is called
        # without _setup_step() in forward_state()
        if step_mode != "linear":
            N *= 2
    ...
    def step(self, u, state):
        """Must have called self._setup_step() and created state\
              with self.default_state() before calling this."""
...
class FFTConv(nn.Module):
    ...
    channels: Can be interpreted as a number of "heads"; the SSM is a map from a 1-dim\
          to C-dim sequence. It's not recommended to change this; instead, \
            increase d_model for larger models.
    ...
    mode: Which kernel algorithm to use. 'nplr' is the full S4 model;\
          'diag' is the simpler S4D. Other options can be found \
            in the kernel registry.

    kernel_args: See the class .kernel.SSMKernel for the kernel constructor\
          which accepts kernel_args. Relevant options that are worth considering\
              and tuning include "mode", "init", "dt_min", "dt_max", "lr"
    """
    ...
    def forward(
        ...
        if self.bidirectional:
            ...
            # This is a deliberate choice since the off-by-one \
            # should not affect any applications
        ...
        # In principle, we could pad to l_kernel+L-1 instead of l_kernel+L,
        # but we choose the latter for
        # equational simplicity. Additionally,
        # we have not experimented to compare the efficiency of the two.
...
class S4Block(nn.Module):
    """General block design wrapping an inner layer. Currently only layer=FFTConv \
        is supported, but easy to incorporate others.

    Arguments:
    - bottleneck: Reduce dimension of inner layer (e.g. used in GSS).
    - gate: Add multiplicative gating (e.g. used in GSS), which is essentially a \
        multiplicative instead of additive residual branch.
    - gate_act: Activation function to apply on the gate residual branch.
    - mult_act: Activation function to apply after gate \
        multiplication (e.g. GELU in GSS).
    - final_act: Activation function to apply after final linear layer.\
          'id' for no activation, None for no linear layer at all.

    - initializer: Initializer on final linear layer.
    - weight_norm: Weight normalization on final linear layer.
    - dropout: standard dropout argument. tie_dropout=True ties the \
        dropout mask across the sequence length, emulating nn.Dropout1d

    - transposed: Choose backbone axis ordering of (B, L, H) (if False) or (B, H, L)\
          (if True) [B=batch size, L=sequence length, H=model dimension]
    ...
    def __init__(
        ...
        **layer_args,  # Arguments into inner layer (e.g. FFTConv)
        ...
        if postact is not None:
            ...
            log.warning(
                "Warning: 'postact' option changed to 'final_act' and will be removed \
                    in a future version."
            )
    ...
    def step(self, x, state):
        """Step one time step as a recurrent model. Intended to \
